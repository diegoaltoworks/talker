...
import { buildChatRequestBody, resolveOpenAIRequestConfig } from "./openai-request";
 ...
 * isn't silently bypassed, aborts after `processing.requestTimeoutMs` so a
 * hung upstream request can't hold a /call, /sms or /whatsapp webhook open
 * indefinitely, and samples at `processing.temperature` - callers already
 * fall back to the original text on any rejection here.
 */
export async function callOpenAI(
  ...
  const { apiUrl, timeoutMs, temperature } = resolveOpenAIRequestConfig(deps.config.processing);
