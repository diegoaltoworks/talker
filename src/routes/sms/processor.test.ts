 ...
 * SMS Processor Tests
 *
 * Covers channel-specific flow content delivery: processSms must prefer
 * FlowResult.smsContent over the generic `response` when the flow handler
 * set it, and fall back to `response` otherwise.
 */
...
const { processSms } = await import("./processor");
...
describe("processSms channel-specific flow content", () => {
  afterEach(() => {
    clearAllContexts();
    stopCleanup();
  });

  it("delivers smsContent over response when the flow handler set it", async () => {
    flowResultToReturn = {
      isFlowActive: false,
      flowCompleted: true,
      response: "SAY_TEXT",
    ...
    const twiml = await processSms(deps, registry, "+15551234567", "hi", "+15559876543");

    expect(twiml).toContain("SMS_TEXT");
    expect(twiml).not.toContain("SAY_TEXT");
    await Promise.resolve();
    await Promise.resolve();
    expect(events).toContain("SMS_TEXT");
  });

  it("falls back to response when the flow handler omits smsContent", async () => {
    flowResultToReturn = {
      isFlowActive: false,
      flowCompleted: true,
      response: "SAY_ONLY",
    ...
    const twiml = await processSms(deps, registry, "+15551234568", "hi", "+15559876543");

    expect(twiml).toContain("SAY_ONLY");
  });

  it("discards smsContent for a failed flow and sends the generic error phrase instead", async () => {
    flowResultToReturn = {
      isFlowActive: false,
      flowCompleted: true,
      response: "SAY_TEXT",
    ...
    const twiml = await processSms(deps, registry, "+15551234569", "hi", "+15559876543");
